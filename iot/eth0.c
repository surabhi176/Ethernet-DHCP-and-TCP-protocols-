...
uint32_t sum;
uint8_t macAddress[HW_ADD_LENGTH] = {2,3,4,5,6,188};
...
bool etherIsArpResponse(uint8_t packet[])
{
    ...
    while (ok & (i < IP_ADD_LENGTH))
    {
        ok = (arp->destIp[i] == dhcpack.DHCPipAdd[i]);
        i++;
    }
    if (ok)
        ok = (arp->op == htons(2));
...
void sendDHCPmessage(uint8_t packet[])
{
    uint16_t tmp16;
    ...
    udp->destPort=htons(67);//from rfc2131
    ...
    dhcp->options[0]=53;//dhcp message type-discover
    ...
    dhcp->options[11]=7;//length
    dhcp->options[12]=0x01;//hardware type-ethernet
    dhcp->options[13]=0x01;//Client MAC address
    dhcp->options[14]=0x07;
    dhcp->options[15]=0x00;
    dhcp->options[16]=0x06;
    dhcp->options[17]=0x13;
    dhcp->options[18]=0x60;
...
bool isDHCPOffer(uint8_t packet[])
{
    ...
    if((dhcp->options[0] == 53) && (dhcp->options[2] == DHCPOFFER))
    {
        return true;
    }
    return false;
...
void collectDHCPoffer(uint8_t packet[])
{
    etherFrame* ether = (etherFrame*)packet;
    ipFrame* ip = (ipFrame*)&ether->data;
    udpFrame* udp = (udpFrame*)((uint8_t*)ip + ((ip->revSize & 0xF) * 4));
    ...
    for (i = 0; i < HW_ADD_LENGTH; i++)
    {
        dhcpoffer.etherAdd[i] = ether->sourceAddress[i];
    }


    for (i = 0; i < IP_ADD_LENGTH; i++)
    {
        dhcpoffer.ipAdd[i] = ip->sourceIp[i];
    }


    for (i = 0; i < IP_ADD_LENGTH; i++)
    {
        dhcpoffer.DhcpYiAdd[i] = dhcp->yiaddr[i];
    }

    for (i = 0; i < IP_ADD_LENGTH; i++)
    {
        dhcpoffer.DhcpSiAdd[i] = dhcp->siaddr[i];
    }


    for (i = 0; i < IP_ADD_LENGTH; i++)
    {
        dhcpoffer.DhcpGiAdd[i] = dhcp->giaddr[i];
    }
    ...
    while(dhcp->options[i]!=255)
    {
        if(dhcp->options[i]==1)
        {
            ...
            for (j = 0; j < len; j++)
            {
                dhcpoffer.DhcpSn[j] = dhcp->options[++i];

            }
        }
        else if(dhcp->options[i]==3)
        {
            len = dhcp->options[++i];

            for (j = 0; j < len; j++)
            {
                dhcpoffer.DhcpRout[j] = dhcp->options[++i];

            }
        }
        else if(dhcp->options[i]==51)
        {
            len = dhcp->options[++i];
            dhcpoffer.DhcpOffLeaseTime=0;
            temp=dhcp->options[++i];
            time=temp<<24;
            temp=dhcp->options[++i];
            temp=temp<<16;
            ...
            temp=dhcp->options[++i];
            time=time+temp;
            dhcpoffer.DhcpOffLeaseTime=time;
        }
        else if(dhcp->options[i]==54)
        {
            len = dhcp->options[++i];

            for (j = 0; j < len; j++)
            {
                dhcpoffer.DhcpSerId[j] = dhcp->options[++i];
            }
            i++;
        }
        else if(dhcp->options[i]==46)
        {
            i+=3;
        }
        else
...
void sendDHCPReq(uint8_t packet[])
{
    uint16_t tmp16;
       ...
       dhcp->magicCookie=0x63538263;
              ...
              dhcp->options[8] = 0x00;
              dhcp->options[9]=0x06;
              dhcp->options[10] = 0x13;
              dhcp->options[11] = 0x60;


              dhcp->options[12] = 50;// requested IP address
              dhcp->options[13] = 4;
              dhcp->options[14] = dhcpoffer.DhcpYiAdd[0];
              dhcp->options[15] = dhcpoffer.DhcpYiAdd[1];
              dhcp->options[16] = dhcpoffer.DhcpYiAdd[2];
              dhcp->options[17] = dhcpoffer.DhcpYiAdd[3];

              dhcp->options[18] = 54;// DHCP server identifier
              dhcp->options[19] = 4;
              dhcp->options[20] = dhcpoffer.DhcpSerId[0];
              dhcp->options[21] = dhcpoffer.DhcpSerId[1];
              dhcp->options[22] = dhcpoffer.DhcpSerId[2];
              dhcp->options[23] = dhcpoffer.DhcpSerId[3];
...
bool isDHCPAck(uint8_t packet[])
{
    ...
    if((dhcp->options[0] == 53) && (dhcp->options[2] == DHCPACK))
    {
        return true;
    }
    return false;
...
void collectDHCPack(uint8_t packet[])
{
    etherFrame* ether = (etherFrame*)packet;
    ipFrame* ip = (ipFrame*)&ether->data;
    udpFrame* udp = (udpFrame*)((uint8_t*)ip + ((ip->revSize & 0xF) * 4));
...
//    }
    ...
    for (i = 0; i < IP_ADD_LENGTH; i++)
    {
        ipAddress[i]=dhcpack.DHCPipAdd[i]=dhcpoffer.DhcpYiAdd[i] = dhcp->yiaddr[i];
...
//    }
    ...
    for (i = 0; i < IP_ADD_LENGTH; i++)
    {
        ipGwAddress[i]=dhcpack.DHCPgwAdd[i]=dhcpoffer.DhcpGiAdd[i] = dhcp->giaddr[i];
    ...
    while(dhcp->options[i]!=255)
    {
        if(dhcp->options[i]==1)
        {
            ...
            for (j = 0; j < len; j++)
            {
                ipSubnetMask[j]=dhcpack.DHCPSn[j]=dhcpoffer.DhcpSn[j] = dhcp->options[++i];
...
bool isDHCPNack(uint8_t packet[])
{
    etherFrame* ether = (etherFrame*)packet;
        ...
        if((dhcp->options[0] == 53) && (dhcp->options[2] == DHCPNACK))
        {
//            state=DHCPNACK;
            return true;
        }
...
void sendDHCPRefresh(uint8_t packet[])
{
    uint16_t tmp16;
    ...
    for(i=0;i<IP_ADD_LENGTH;i++)
    {
        dhcpack.DHCPipAdd[i]=dhcp->yiaddr[i];
    }
    dhcp->magicCookie=0x63538263;
    dhcp->options[0]=53;//dhcp message type-discover
    dhcp->options[1]=1;//length
...
bool isTcpListen(uint8_t packet[])
{
    etherFrame* ether = (etherFrame*)packet;
    ipFrame* ip = (ipFrame*)&ether->data;
    ...
    bool ok=false;
//    uint8_t flags;
    tcp->DoResFlgs=htons(tcp->DoResFlgs);
// flags=tcp->DoResFlgs & 0xFF;
 if(ip->protocol==6)
 {
     if((tcp->DoResFlgs & 2)==2)
     {
        ok=true;
     }
 }
...
void sendTCPSynAck(uint8_t packet[])
{
    etherFrame* ether = (etherFrame*)packet;
    ipFrame* ip = (ipFrame*)&ether->data;
    ip->revSize = 0x45;
    tcpFrame* tcp = (tcpFrame*)((uint8_t*)ip + ((ip->revSize & 0xF) * 4));
    tcpOptions* tcp_options=(tcpOptions*)(tcp+(20));
    uint16_t tmp16;
    uint8_t i;
    ...
    for(i=0;i<4;i++)
    {
        tempSrcIp[i]=ip->sourceIp[i];
        ip->sourceIp[i]=ip->destIp[i];
        ip->destIp[i]=tempSrcIp[i];
    }



    tcp->destPort=(tcp->sourcePort);//from rfc793
    tcp->sourcePort=htons(23);

//    tcp->seqNo=htons(0);
    uint32_t seqNum=htons32(tcp->seqNo);
    uint32_t ackNum=seqNum+1;
    tcp->seqNo=tcp->ackNo;//0;
    tcp->ackNo=htons32(ackNum);
    uint8_t offset=6;//24>>2;
    uint8_t flags=18;
    uint16_t doresflgs=0;
    doresflgs=(offset<<12);
    doresflgs+=flags;
    tcp->DoResFlgs=htons(doresflgs);
    ...
    etherSumWords(tcp_options,2);
    etherSumWords(&tcp_options->mssize,2);
    tcp->checksum = getEtherChecksum();



    // send packet with size = ether + udp hdr + ip header + udp_size
    etherPutPacket((uint8_t *)ether, 14 + ((ip->revSize & 0xF) * 4) + 20+3);
}
...
//    ip->flagsAndOffset=0;
    ...
    //    tcp->seqNo=htons(0);
    uint32_t seqNum=htons32(tcp->seqNo);
    uint32_t ackNum=seqNum+payloadSize;
    tcp->seqNo=tcp->ackNo;//0;
    tcp->ackNo=htons32(ackNum);
    uint8_t offset=5;//20>>2;
    uint8_t flags=16;
    uint16_t doresflgs=0;
    doresflgs=(offset<<12);
    doresflgs+=flags;
...
void  sendTelnetMsg(uint8_t packet[])
{
    etherFrame* ether = (etherFrame*)packet;
    ipFrame* ip = (ipFrame*)&ether->data;
    ip->revSize = 0x45;
...
//        ip->destIp[i]=tempSrcIp[i];
    ...
    tcp->destPort=(tcp->destPort);//from rfc793
    tcp->sourcePort=htons(23);
...
//    tcp->ackNo=htons32(ackNum);
    ...
    {
        tcp->data[i]=tcpData[i];
    }


    ip->length = htons(((ip->revSize & 0xF) * 4) +20+dataSize);
    // 32-bit sum over ip header
    sum = 0;
    etherSumWords(&ip->revSize, 10);
    etherSumWords(ip->sourceIp, ((ip->revSize & 0xF) * 4) - 12);
    ...
    etherSumWords(tcp, 16);
    etherSumWords(&tcp->urgentPointer, 2);
        etherSumWords(&tcp->data,dataSize);
    //    etherSumWords(&tcp_options->mssize,2);
    tcp->checksum = getEtherChecksum();

    // send packet with size = ether + udp hdr + ip header + udp_size
    etherPutPacket((uint8_t *)ether, 14 + ((ip->revSize & 0xF) * 4) +20+dataSize);
}
